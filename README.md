...
### Leaflet（リーフレット）とは
オープンソースのWebマップを作成するためのJavaScriptライブラリです。Google ColabのNotebook内でブラウザ表示するために用いられています。
...
通常LeafletではOpen Street Mapが背景に表示されますが、Foliumを使用した際エラーになったため、コード内で国土地理院を指定しています。
...
index.htmlをカスタマイズして、オリジナルマップを作成しましょう！
